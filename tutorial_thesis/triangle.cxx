...
Triangle::Triangle( ShaderProgram *shdrPrg ) : ObjectInstance( shdrPrg ), initialized( false )
{
    ...
    if ( ( shaderProgram != nullptr ) && shaderProgram->initialized &&
         ( shaderProgram->locations.position != -1 ) && ( shaderProgram->locations.PVMmatrix != -1 ) )
    {
...
void Triangle::draw( const glm::mat4 &viewMatrix, const glm::mat4 &projectionMatrix )
{
    if ( initialized && ( shaderProgram != nullptr ) )
    {
        ...
        glUniform4fv( shaderProgram->locations.testingColor, 1, glm::value_ptr( TESTING_COLOR ) ); // XD
